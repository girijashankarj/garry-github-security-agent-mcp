#!/usr/bin/env node
/**
 * Deterministic repository remediation entrypoint.
 *
 * The TypeScript orchestrator supplies operator-approved environment variables.
 * Claude may reason about a plan, but this script is the execution boundary.
 */
...
const required = ['GITHUB_OWNER', 'GITHUB_TOKEN', 'SECURITY_AUDIT_REPOSITORY'];
...
try {
  ...
  const configured = await command('git', ['config', 'user.name', 'github-security-audit[bot]'], cloneDir);
  if (configured.code !== 0) throw new Error(configured.stderr);
  await command('git', ['config', 'user.email', 'github-actions[bot]@users.noreply.github.com'], cloneDir);
